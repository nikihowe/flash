...
for i in x_values:
    ctr = 0
    cut_value = i
    hist = ROOT.TH1D("hist", "Cut Value: " + str(cut_value), 100, -300., 300.)
    while 1:
        print "ctr: " + str(ctr)
        endpoint = getEndpoint(ctr)
        print "end: " + str(endpoint)
        if makesCut(ctr, endpoint, cut_value):
            if abs(ch.mc_start_z - getMaxNpeZ(ctr, endpoint)) > abs(ch.mc_end_z - getMaxNpeZ(ctr, endpoint)):
                hist.Fill(ch.mc_end_z - getMaxNpeZ(ctr, endpoint))
            else:
